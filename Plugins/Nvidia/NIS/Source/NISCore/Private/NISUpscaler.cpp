...
void FNISViewExtension::BeginRenderViewFamily(FSceneViewFamily& InViewFamily)
{
	...
	if (bIsNISSupported && bIsNISEnabled && (bIsNISUpscalingEnabled || bIsNISSharpeningEnabled))
	{
		...
		*/
		...
		const bool bIsActuallyPrimaryUpscaling = InViewFamily.GetPrimaryResolutionFractionUpperBound() != 1.0f;
		const bool bIsActuallySecondaryUpscaling = InViewFamily.SecondaryViewFraction != 1.0f;
